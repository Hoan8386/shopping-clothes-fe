...
import { DoiCa, LichLamViec, ChiTietLichLam, NhanVien, ReqDoiCaDTO } from "@/types";
import { doiCaService, lichLamViecService, chiTietLichLamService } from "@/services/schedule.service";
import { nhanVienService } from "@/services/employee.service";
import { formatDate } from "@/lib/utils";
import Loading from "@/components/ui/Loading";
import toast from "react-hot-toast";
import { FiPlus, FiEdit2, FiX, FiRepeat } from "react-icons/fi";
...
export default function StaffDoiCaPage() {
  ...
  useEffect(() => {
    ...
    nhanVienService.getAll().then((d) => setEmployees(d ?? [])).catch(() => {});
  }, []);
  ...
  const handleSave = async () => {
    if (!editId && (!form.lichLamViec.id || !form.chiTietLichLam.id || !form.nhanVienNhanCa.id)) {
      toast.error("Vui lòng điền đầy đủ thông tin"); return;
    }
    try {
      setSaving(true);
      if (editId) { await doiCaService.update(form); toast.success("Cập nhật thành công"); }
      else { await doiCaService.create(form); toast.success("Gửi yêu cầu đổi ca thành công"); }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 bg-card rounded-2xl border border-subtle p-4 lg:p-5">
        <div>
          <h1 className="text-lg font-bold text-foreground">Đổi ca</h1>
          <p className="text-sm text-muted mt-1">Tạo và quản lý yêu cầu đổi ca làm việc.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        {[
          ...
          { label: "Chờ duyệt", value: list.filter((l) => l.trangThai === 0).length, color: "text-yellow-600" },
          { label: "Đồng ý", value: list.filter((l) => l.trangThai === 1).length, color: "text-green-600" },
        ].map((s) => (
          <div key={s.label} className="bg-card border border-subtle rounded-xl p-4">
      ...
      {loading ? <Loading /> : list.length === 0 ? (
        <div className="text-center py-16 text-muted">Chưa có yêu cầu đổi ca nào</div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-section border-b border-subtle">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-muted">Ca (ID)</th>
                  <th className="px-4 py-3 text-left font-medium text-muted">NV nhận ca</th>
                ...
                {list.map((item) => (
                  <tr key={item.id} className="hover:bg-section transition">
                    ...
                    <td className="px-4 py-3 text-muted">
                      <span className="flex items-center gap-1.5"><FiRepeat size={13} />#{item.lichLamViec?.id}</span>
                    </td>
                    <td className="px-4 py-3 text-muted">#{item.chiTietLichLam?.id}</td>
                    <td className="px-4 py-3 font-medium text-foreground">{item.nhanVienNhanCa?.tenNhanVien ?? "—"}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(item.trangThai)}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-center">
                        {item.trangThai === 0 && (
                          <button onClick={() => openEdit(item)} className="p-1.5 text-blue-500 hover:bg-blue-500/10 rounded" title="Cập nhật">
                            <FiEdit2 size={15} />
                          </button>
                        )}
                      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-4 border-b border-subtle">
              <h2 className="font-bold text-lg text-foreground">{editId ? "Cập nhật đổi ca" : "Tạo yêu cầu đổi ca"}</h2>
            ...
            <div className="p-5 space-y-4">
              {!editId && (
                <>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1.5">Nhân viên nhận ca *</label>
                    <select value={form.nhanVienNhanCa.id || ""} onChange={(e) => setForm({ ...form, nhanVienNhanCa: { id: Number(e.target.value) } })}
                      className="w-full h-10 px-3 rounded-lg bg-section border border-subtle text-sm">
            ...
            <div className="flex gap-3 p-4 border-t border-subtle">
              ...
              <button onClick={handleSave} disabled={saving} className="flex-1 py-2.5 bg-accent text-white rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-60">
                {saving ? "Đang lưu..." : editId ? "Cập nhật" : "Gửi yêu cầu"}
              </button>
            </div>
          </div>
        </div>
