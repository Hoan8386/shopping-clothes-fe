...
import { lichLamViecService } from "@/services/schedule.service";
import { nhanVienService } from "@/services/employee.service";
import { formatDate } from "@/lib/utils";
import Loading from "@/components/ui/Loading";
import toast from "react-hot-toast";
import { FiPlus, FiTrash2, FiX, FiUpload, FiDownload, FiCalendar } from "react-icons/fi";
...
export default function StaffLichLamViecPage() {
  ...
  const handleSave = async () => {
    ...
    } catch { toast.error("Thêm thất bại"); } finally { setSaving(false); }
  };
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      a.href = url; a.download = "lich-lam-viec-mau.xlsx"; a.click();
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 bg-card rounded-2xl border border-subtle p-4 lg:p-5">
        <div>
          ...
          <p className="text-sm text-muted mt-1">Xem và quản lý lịch làm việc. Hỗ trợ import từ Excel.</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleDownloadTemplate} className="flex items-center gap-2 px-3 py-2 border border-subtle text-sm rounded-xl hover:bg-section transition">
            <FiDownload size={15} /> Tải mẫu
          </button>
              ...
              <tbody className="divide-y divide-subtle">
                {list.map((lich) => (
                  <tr key={lich.id} className="hover:bg-section transition">
                    ...
                    <td className="px-4 py-3 font-medium text-foreground">{lich.nhanVien?.tenNhanVien ?? "—"}</td>
                    <td className="px-4 py-3 text-muted">
                      <span className="flex items-center gap-1.5"><FiCalendar size={13} />{lich.ngayLamViec}</span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        lich.trangThai === 1 ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
                      }`}>{lich.trangThai === 1 ? "Đang làm" : "Nghỉ"}</span>
                    </td>
                    <td className="px-4 py-3 text-muted">{formatDate(lich.ngayTao ?? "")}</td>
                    <td className="px-4 py-3">
                      <div className="flex justify-center">
                        <button onClick={() => setDeleteId(lich.id)} className="p-1.5 text-red-500 hover:bg-red-500/10 rounded"><FiTrash2 size={15} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-md">
            ...
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">Nhân viên *</label>
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">Ngày làm việc</label>
                <input type="date" value={form.ngayLamViec} onChange={(e) => setForm({ ...form, ngayLamViec: e.target.value })}
                  className="w-full h-10 px-3 rounded-lg bg-section border border-subtle text-sm" />
              </div>
              <div>
