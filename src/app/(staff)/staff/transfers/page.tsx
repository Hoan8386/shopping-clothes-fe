...
import { ImportGoodsDialog } from "@/components/transfer/ImportGoodsDialog";
...
export default function AdminTransfersPage() {
  ...
  const openAction = (item: DonLuanChuyen, type: 1 | 2 | 3 | 4) => {
    setActionItem(item);
    setActionType(type);
    setShowConfirm(true);
  };
  ...
  const pendingCount = transfers.filter(
    (r) => r.trangThai === "Chờ xác nhận" || r.trangThai === "Chờ xử lý",
  ).length;
  const confirmedCount = transfers.filter(
    (r) => r.trangThai === "Đã xác nhận",
  ).length;
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              {selected.cuaHangGuiId === currentStoreId &&
                selected.trangThai === "Đã xác nhận" && (
                  <div className="flex gap-3 pt-3">
                    <button
                      onClick={() => {
                        setShowDetail(false);
              ...
              {selected.cuaHangDatId === currentStoreId &&
