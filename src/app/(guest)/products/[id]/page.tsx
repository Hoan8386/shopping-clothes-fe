...
import { productService, productVariantService } from '@/services/product.service';
import { danhGiaService } from '@/services/common.service';
import { cartService } from '@/services/cart.service';
import { useAuthStore } from '@/store/auth.store';
import { useCartStore } from '@/store/cart.store';
import { ResSanPhamDTO, ResChiTietSanPhamDTO, ResDanhGiaSanPhamDTO } from '@/types';
...
export default function ProductDetailPage() {
    ...
    const [selectedVariant, setSelectedVariant] = useState<ResChiTietSanPhamDTO | null>(null);
    ...
    if (!product) {
        return <div className="text-center py-20 text-muted">Không tìm thấy sản phẩm</div>;
    }
    ...
    const avgRating =
        reviews.length > 0 ? reviews.reduce((sum, r) => sum + r.soSao, 0) / reviews.length : 0;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                            {/* Quantity */}
                            <div className="flex items-center rounded-xl border border-subtle overflow-hidden shrink-0">
                                <button
                                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                                    className="w-14 h-14 flex items-center justify-center hover:bg-section transition-colors text-foreground"
                                >
                                    <FiMinus size={14} />
                                </button>

                                <span className="w-16 h-14 flex items-center justify-center text-sm font-bold border-x border-subtle bg-card">
                                    {quantity}
                                </span>
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                                <button
                                    ...
                                    className="w-full sm:w-[220px] bg-accent hover:bg-accent-hover text-white px-6 py-3.5 rounded-xl text-sm font-bold uppercase tracking-wider disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2.5 shadow-lg shadow-accent/20 hover:shadow-accent/30"
                                >
                                ...
                                <button
                                    ...
                                    className="w-full sm:w-[220px] bg-foreground hover:opacity-90 text-background px-6 py-3.5 rounded-xl text-sm font-bold uppercase tracking-wider disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center gap-2.5 shadow-lg"
                                >
                                    Thêm và thử đồ
                                </button>
                            </div>
