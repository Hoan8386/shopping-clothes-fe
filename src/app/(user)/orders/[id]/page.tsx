...
import { FiArrowLeft, FiStar, FiX, FiCamera, FiVideo, FiRotateCcw, FiRepeat } from 'react-icons/fi';
...
export default function OrderDetailPage() {
    ...
    const [exchangeVariants, setExchangeVariants] = useState<ResChiTietSanPhamDTO[]>([]);
    ...
    const returnedQuantityByItemId = useMemo(() => {
        ...
        const isRejectedStatus = (status?: string) =>
            status?.toLowerCase().includes('từ chối') || status?.toLowerCase().includes('tu choi');
        ...
        for (const ret of returnHistory) {
            ...
            for (const detail of ret.chiTietTraHangs || []) {
                if (detail.chiTietDonHangId) {
                    ...
                    quantityMap.set(detail.chiTietDonHangId, current + (detail.soLuong || 0));
                }
            }
        }
            ...
            try {
                ...
                if (data.trangThai === 'Đã nhận hàng' && data.chiTietDonHangs) {
                    ...
                    for (const item of data.chiTietDonHangs) {
                        if (item.id) {
                            try {
                                const reviews = await danhGiaService.getByChiTietDonHang(item.id);
                ...
                if (data.trangThai === 'Đã nhận hàng') {
                    try {
                        const returns = await traHangService.getByDonHangId(Number(params.id));
                        setReturnHistory(returns);
                    } catch {
                        // No returns yet
                    }
                    try {
                        const exchanges = await doiHangService.getByDonHangId(Number(params.id));
    ...
    const handleSubmitReturn = async () => {
        ...
        if (selected.some((i) => i.soLuongTra < 1 || i.soLuongTra > i.soLuongConLai)) {
            ...
            await traHangService.create(
                {
                    ...
                    thongTinChuyenKhoan: refundMethod === 1 ? refundBankInfo.trim() : undefined,
    ...
    return (
        <>
            ...
            <div className="bg-section py-8 text-center">
                <h2 className="text-3xl font-extrabold text-foreground mb-1">Chi tiết đơn hàng</h2>
            ...
            <div className="max-w-5xl mx-auto px-4 py-10">
                ...
                {refreshing && (
                    <p className="text-xs text-gray-500 mb-4">Đang cập nhật đơn hàng...</p>
                )}
                ...
                <div className="border border-subtle bg-card mb-6">
                    <div className="flex items-center justify-between px-6 py-5 border-b border-subtle">
                        <h3 className="text-lg font-bold text-foreground">Đơn hàng #{order.id}</h3>
                        <span
                        ...
                        <div className="space-y-3 text-sm">
                            ...
                            {order.khuyenMaiHoaDon && (
                                <div className="flex justify-between text-green-600 dark:text-green-400">
                                    ...
                                    <span>-{formatCurrency(order.khuyenMaiHoaDon.tienDaGiam)}</span>
                                </div>
                            )}
                            {order.khuyenMaiDiem && (
                                <div className="flex justify-between text-green-600 dark:text-green-400">
                ...
                {(order.trangThai === 'Chờ xác nhận' ||
                    ...
                    <div className="flex justify-end gap-3 mb-6">
                        ...
                        {(order.trangThai === 'Đang giao hàng' || order.trangThai === 3) && (
                            <button
                                ...
                                <div key={ret.id} className="px-6 py-4">
                                    ...
                                    <div className="mt-3 text-right">
                                        <span className="text-sm text-gray-500">
                                            Tổng tiền còn lại của đơn hàng:{' '}
                                        </span>
                                        ...
                                        {ex.chiTietDoiHangs?.map((ct) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-3 text-sm">
                                                    ...
                                                    <p className="font-semibold text-foreground">
                                                        {formatCurrency(
                                                            ct.giaSanPhamTra * ct.soLuongTra,
                                                        )}
                                                    </p>
                                                </div>
                                                {/* Sản phẩm đổi */}
                                                <div className="flex items-center gap-3 text-sm">
                                                    ...
                                                    <p className="font-semibold text-foreground">
                                                        {formatCurrency(
                                                            ct.giaSanPhamDoi * ct.soLuongTra,
                                                        )}
                                                    </p>
                                                </div>
                                                {/* Chênh lệch */}
