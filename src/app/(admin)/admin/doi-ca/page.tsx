...
export default function AdminDoiCaPage() {
  ...
  const handleSave = async () => {
    if (!form.lichLamViec.id || !form.chiTietLichLam.id || !form.nhanVienNhanCa.id) {
    ...
    try {
      ...
      else { await doiCaService.create(form); toast.success("Tạo yêu cầu đổi ca thành công"); }
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 bg-card rounded-2xl border border-subtle p-4 lg:p-5">
        <div>
          ...
          <p className="text-sm text-muted mt-1">Quản lý yêu cầu đổi ca giữa các nhân viên.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: "Từ chối", value: list.filter((l) => l.trangThai === 2).length, color: "text-red-500" },
        ].map((s) => (
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl shadow-xl w-full max-w-sm p-6">
            <h3 className="font-bold text-lg text-foreground mb-2">Xóa yêu cầu đổi ca?</h3>
