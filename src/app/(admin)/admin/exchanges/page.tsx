...
export default function AdminExchangesPage() {
  ...
  const approvedCount = exchanges.filter(
      ...
      <div className="flex items-center justify-between bg-card rounded-2xl border border-subtle p-4 lg:p-5">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
            Quản lý tất cả phiếu đổi hàng từ khách hàng.
          </p>
        </div>
        <div className="flex items-center gap-2 text-sm text-muted">
          <FiRepeat size={16} />
          Tổng: {exchanges.length} phiếu
        </div>
      </div>
          ...
          <div className="bg-card rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            {actionType === 1 ? (
              <p className="text-sm text-green-700 bg-green-50 rounded-lg p-3 mb-4">
                Sau khi duyệt, yêu cầu đổi hàng sẽ được chấp nhận và khách hàng
                sẽ nhận được sản phẩm mới.
              </p>
            ) : (
              <p className="text-sm text-red-700 bg-red-50 rounded-lg p-3 mb-4">
                Sau khi từ chối, yêu cầu đổi hàng sẽ bị hủy. Hành động này không
                thể hoàn tác.
              </p>
            )}
            <div className="flex gap-3">
